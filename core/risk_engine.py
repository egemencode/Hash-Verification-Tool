...
class RiskFactor:
    ...
    def to_dict(self) -> dict[str, Any]:
        # Keys and parameters, not words. A consumer that wants prose renders
        # it; a consumer that wants to *act* on the finding gets something
        # stable to match on, which a translated sentence would not be.
        return {
...
def _score_virustotal(vt: Optional[VTLookupResult], out: list[RiskFactor]) -> int:
    if vt is None:
        out.append(_vt_factor(phrase("factor.vt.not_queried"), 0, "info"))
        return 0
    ...
    if vt.status == VTStatus.OK:
        ...
        if vt.analysing_engines <= 0:
            out.append(
                _vt_factor(phrase("factor.vt.no_analysing_engines"), 0, "info")
            )
            return 0
        ...
        if malicious >= 5:
            ...
            detail = phrase("factor.vt.malicious", count=malicious)
        elif malicious >= 1:
            weight, severity = 50, "bad"
            detail = phrase("factor.vt.malicious", count=malicious)
        elif suspicious >= 3:
            weight, severity = 25, "warn"
            detail = phrase("factor.vt.suspicious", count=suspicious)
        elif suspicious >= 1:
            ...
            if not vt.is_fresh:
                # Two whole sentences rather than one with a clause spliced
                # in: a translation has to be free to reorder them.
                reason = (
                    "factor.vt.stale" if vt.is_stale else "factor.vt.undated"
                )
                out.append(_vt_factor(phrase(reason), 0, "info"))
                return 0
            if vt.stats_malformed:
                out.append(
                    _vt_factor(phrase("factor.vt.malformed"), 0, "info")
                )
                return 0
            ...
            detail = phrase("factor.vt.clean", count=vt.analysing_engines)
        out.append(_vt_factor(detail, weight, severity))
        return weight

    if vt.status == VTStatus.NOT_FOUND:
        out.append(
            _vt_factor(phrase("factor.vt.not_found"), 10, "warn")
        )
        return 10

    # No key / unauthorized / rate-limited / network / other error: these are
    # NOT clean evidence, and they carry no weight.
    # The service's own wording is worth keeping when there is any, so it is
    # carried as a value inside a translated frame rather than replacing the
    # sentence — otherwise the only readable part of the row is English by
    # accident of who returned the error.
    if vt.message:
        detail = phrase("factor.vt.reported", message=vt.message)
    else:
        detail = phrase({
            VTStatus.NO_API_KEY: "factor.vt.no_key",
            VTStatus.UNAUTHORIZED: "factor.vt.unauthorized",
            VTStatus.RATE_LIMITED: "factor.vt.rate_limited",
            VTStatus.NETWORK_ERROR: "factor.vt.network_error",
        }.get(vt.status, "factor.vt.failed"))
    out.append(_vt_factor(detail, 0, "info"))
    return 0


def _vt_factor(detail: Phrase, weight: int, severity: str) -> RiskFactor:
    return RiskFactor(phrase("factor.name.virustotal"), detail, weight, severity)


def _sig_factor(detail: Phrase, weight: int, severity: str) -> RiskFactor:
    return RiskFactor(phrase("factor.name.signature"), detail, weight, severity)


def _local_factor(detail: Phrase, weight: int, severity: str) -> RiskFactor:
    return RiskFactor(phrase("factor.name.local"), detail, weight, severity)


def _score_signature(sig: Optional[SignatureResult], out: list[RiskFactor]) -> int:
    if sig is None:
        out.append(_sig_factor(phrase("factor.sig.not_checked"), 0, "info"))
        return 0

    status = sig.status
    if status == SignatureStatus.SIGNED_VALID:
        detail = (
            phrase("factor.sig.signed_valid_by", signer=sig.signer)
            if sig.signer
            else phrase("factor.sig.signed_valid")
        )
        ...
        out.append(_sig_factor(detail, 0, "good"))
        return 0

    if status == SignatureStatus.HASH_MISMATCH:
        out.append(
            _sig_factor(phrase("factor.sig.hash_mismatch"), 60, "bad")
        )
        return 60

    if status == SignatureStatus.UNTRUSTED:
        out.append(
            _sig_factor(phrase("factor.sig.untrusted"), 30, "bad")
        )
        return 30

    if status == SignatureStatus.UNSIGNED:
        out.append(
            _sig_factor(phrase("factor.sig.unsigned"), 8, "warn")
        )
        return 8

    if status == SignatureStatus.NOT_APPLICABLE:
        out.append(
            _sig_factor(phrase("factor.sig.not_applicable"), 0, "info")
        )
        return 0

    if status == SignatureStatus.UNSUPPORTED:
        out.append(
            _sig_factor(phrase("factor.sig.unsupported"), 0, "info")
        )
        return 0
    ...
    out.append(
        _sig_factor(
            phrase("factor.sig.reported", message=sig.message)
            if sig.message
            else phrase("factor.sig.unknown"),
            0,
            "info",
        )
    )
...
def _score_local(local: Optional[LocalVerifyResult], out: list[RiskFactor]) -> int:
    ...
    if local.status == LocalVerifyStatus.SAME:
        ...
        out.append(
            _local_factor(phrase("factor.local.same"), 0, "good")
        )
        return 0
    ...
    if local.status == LocalVerifyStatus.NEW:
        out.append(
            _local_factor(phrase("factor.local.new"), 0, "info")
        )
        return 0
...
# Headline
# ----------------------------------------------------------------------
def _headline_for(level: RiskLevel) -> Phrase:
    # Every wording of these avoids "safe". No tool can guarantee a file is
    # harmless — we only report what the signals say — and that constraint has
    # to survive translation, so it is written into each entry rather than
    # left to whoever adds the next language.
